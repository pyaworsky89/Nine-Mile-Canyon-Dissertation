...
#gamm
mod1<-gamm(meanGDE~s(avgprecip)+s(spd)+s(aors,bs="re"),data=df,correlation=corAR1())
summary(mod1$gam)

#hist(resid(mod1$gam))
acf(resid(mod1$lme,type="normalized"),lag.max=1000)

plot(mod1$gam,shift=mod1$gam$coefficients[1],shade=T,select=2,seWithMean=TRUE,ylab="Mean GDE")
plot(mod1$gam,shift=mod1$gam$coefficients[1],shade=T,select=1,seWithMean=TRUE,ylab="Mean GDE")
...
##gammmm
mod1<-gamm(meanprobscul~s(spd)+s(avgprecip)+s(aors,spd,bs="re")+s(aors,bs="re"),data=df,correlation=corAR1())
summary(mod1$gam)
acf(resid(mod1$lme,type="normalized"),lag.max=1000)
...
#gamm
mod1<-gamm(numstor~s(avgprecip)+s(spd,k=2)+s(meanGDE),data=df,correlation=corAR1())
summary(mod1$gam)
...
plot(mod1$gam,select=3,shade=T,residuals=T,ylab="Num Stor",xlab="GDE",shift= ilink(coef(mod1$gam)[1]))
...
#gamm
mod1<-gamm(VS~s(avgprecip)+s(spd,k=2)+s(meanGDE),data=df,correlation=corAR1())
summary(mod1$gam)
#hist(resid(mod1$gam),main="NumStor~SPD+AvgPrecip")
acf(resid(mod1$lme,type="normalized"),lag.max=1000)
...
plot(mod1$gam,select=3,shade=T,residuals=T,ylab="VS",xlab="GDE",shift= ilink(coef(mod1$gam)[1]))
...
#gamm
mod1<-gamm(elevfp~s(avgprecip)+s(spd,k=3)+s(meanGDE),data=df,correlation=corAR1())
summary(mod1$gam)
#hist(resid(mod1$gam),main="NumStor~SPD+AvgPrecip")
acf(resid(mod1$lme,type="normalized"),lag.max=1000)
...
plot(mod1$gam,select=3,shade=T,residuals=T,ylab="Elev",xlab="GDE",shift= ilink(coef(mod1$gam)[1]))
```

## Results!
### Number of storage ~ precip, population, and habitat suitability

### VS ~ precip, population, and habitat suitability
### Elevation ~ precip, population, and habitat suitability
