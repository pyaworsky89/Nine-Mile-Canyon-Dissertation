...
plot(BchronCalibrate(ages=c(sort(stors$uncal_age,decreasing=F)),
                       ...
                       calCurves=rep('intcal20',nrow(stors))),withPositions=T)
```
...
par(mfrow=c(1,3))
#gamm
mod1<-gamm(VS~s(avgprecip)+s(spd)+aors,data=df,correlation=corAR1())
summary(mod1$gam)
#hist(resid(mod1$gam),main="NumStor~SPD+AvgPrecip")
acf(resid(mod1$lme,type="normalized"))
...
plot(mod1$gam,select=2,shade=T,ylab="Viewshed of Floodplain",xlab="SPD",shift= coef(mod1$gam)[1],residuals=T);box() 

plot(mod1$gam,select=1,shade=T,ylab="Viewshed of Floodplain",xlab="Precip",shift= coef(mod1$gam)[1],residuals=T,ylim=c(0,4000));box() 
#,ylim=c(0,4000)
###
#deviance explained
