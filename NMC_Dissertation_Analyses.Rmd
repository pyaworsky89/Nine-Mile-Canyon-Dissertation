...
plot(mod1$gam,select=1,shade=T,ylab="Probs Cul",xlab="SPD",shift= coef(mod1$gam)[1],residuals=T)
...
for (i in 1:nrow(stors)){
  ...
  x<-data.frame(yearBP=dates$grids$'1'$calBP,gde=1)
                #*dates$grids$"1"$PrDens) #1 times the prob density in an attempt to weight that number of storage features
...
#gamm
mod1<-gamm(numstor~s(avgprecip)+s(spd)+s(meanprobscul)+aors,data=df,correlation=corAR1())
summary(mod1$gam)
...
plot(mod1$gam,select=2,shade=T,ylab="Num Stor",xlab="SPD",shift=coef(mod1$gam)[1],residuals=T);box()
...
#gamm
mod1<-gamm(VS~s(avgprecip)+s(spd)+s(meanprobscul)+aors,data=df,correlation=corAR1())
summary(mod1$gam)
#hist(resid(mod1$gam),main="NumStor~SPD+AvgPrecip")
acf(resid(mod1$lme,type="normalized"),lag.max=1000)
...
plot(mod1$gam,select=2,shade=T,ylab="Viewshed of Floodplain",xlab="SPD",shift= coef(mod1$gam)[1],residuals=T);box() #this plot is close but not exact
###
#deviance explained
...
#gamm
mod1<-gamm(elevfp~s(avgprecip)+s(spd)+s(meanprobscul)+aors,data=df,correlation=corAR1())
summary(mod1$gam)
#hist(resid(mod1$gam),main="NumStor~SPD+AvgPrecip")
acf(resid(mod1$lme,type="normalized"),lag.max=1000)
...
plot(mod1$gam,select=2,shade=T,ylab="Elevation above Floodplain",xlab="SPD",shift= coef(mod1$gam)[1],residuals=T);box() #this plot is close but not exact...
